...
export function AuthSplitLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex min-h-0 w-full flex-1 flex-col bg-white md:min-h-svh md:flex-row md:items-stretch">
      {/* Hero: one image — mobile strip + desktop portrait frame (avoids duplicate src + wrong `sizes`) */}
      <div className="relative w-full shrink-0 md:flex md:min-h-0 md:w-1/2 md:min-w-0 md:flex-col md:items-center md:justify-center md:bg-white md:px-5 md:py-5 lg:px-6 lg:py-6 xl:px-8 xl:py-8">
        <div className="relative h-52 w-full overflow-hidden sm:h-60 md:h-[min(90svh,920px)] md:w-[min(100%,min(920px,calc(min(90svh,920px)*3/4*1.15)))] md:rounded-[28px] md:shadow-[0_2px_8px_rgba(15,23,42,0.08)] lg:rounded-[32px]">
          <Image
            src={AUTH_HERO_SRC}
            alt=""
            fill
